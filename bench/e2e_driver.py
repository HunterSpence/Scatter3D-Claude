...
phase = sys.argv[5] if len(sys.argv) > 5 else "all"  # sim (MPI-friendly) | post (single rank) | all
...
if phase in ("all", "post"):
    # solveFromQs is serial (COMM_SELF/numpy) - run this phase with a single rank
