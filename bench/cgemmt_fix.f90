...
! OpenBLAS 0.3.26's cgemmt kernel segfaults when called from ZMUMPS 5.8.2 LDLT
...
! referenced triangle. LD_PRELOAD it over libblas so only cgemmt_ is overridden.
subroutine cgemmt(uplo, transa, transb, n, k, alpha, a, lda, b, ldb, beta, c, ldc)
  implicit none
  ...
  complex*8, parameter :: zone = (1.0, 0.0), zzero = (0.0, 0.0)
  ...
  do j1 = 1, n, nbmax
    ...
    if (notra) then
      if (lsame(transb, 'N')) then
        call cgemm(transa, transb, m1, jb, k, zone, a(r0, 1), lda, b(1, j1), ldb, zzero, w, n)
      else
        call cgemm(transa, transb, m1, jb, k, zone, a(r0, 1), lda, b(j1, 1), ldb, zzero, w, n)
      end if
    else
      if (lsame(transb, 'N')) then
        call cgemm(transa, transb, m1, jb, k, zone, a(1, r0), lda, b(1, j1), ldb, zzero, w, n)
      else
        call cgemm(transa, transb, m1, jb, k, zone, a(1, r0), lda, b(j1, 1), ldb, zzero, w, n)
      end if
    end if
    do jj = 1, jb
