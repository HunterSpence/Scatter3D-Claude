...
SCATT3D_SRC=/work/hunter-Scatt3D/Scatt3D timeout 1200 python3 bench_driver.py $H $DEG $NF '{}' fast 2>&1 | grep -E 'BENCH_RESULT|1st solution|NaN|Traceback|Error' | head -5
echo "=== MODIFIED + BLR (tol 1e-6) ==="
SCATT3D_SRC=/work/hunter-Scatt3D/Scatt3D timeout 1200 python3 bench_driver.py $H $DEG $NF '{"blr_tol": 1e-6}' blr 2>&1 | grep -E 'BENCH_RESULT|1st solution|NaN|Traceback|Error' | head -5
echo "=== MODIFIED sweep mode (anchor LU + FGMRES) ==="
SCATT3D_SRC=/work/hunter-Scatt3D/Scatt3D timeout 1200 python3 bench_driver.py $H $DEG $NF '{"sweep_mode": true}' sweep 2>&1 | grep -E 'BENCH_RESULT|1st solution|Sweep mode|NaN|Traceback|Error' | head -8
